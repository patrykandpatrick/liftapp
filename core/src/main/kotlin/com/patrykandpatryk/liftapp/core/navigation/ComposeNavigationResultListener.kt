...
class ComposeNavigationResultListener(private val navController: NavController) :
    ...
    override suspend fun <T> registerResultListener(key: String, listener: (result: T) -> Unit) {
        ...
        currentBackStackEntry.savedStateHandle.getStateFlow<T?>(key, null).collect { result ->
            if (result != null) {
                listener(result)
