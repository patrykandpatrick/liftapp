...
class UnitConverterTest {
    ...
    private val testedValues = arrayOf(1.0, 4.2, 9.99, 69.0, 123.45)
    ...
    fun `convert kg to kg`() = runBlocking {
        testedValues.forEach { testedValue ->
            testedValue.convert(from = MassUnit.Kilograms, to = MassUnit.Kilograms)
        }
    }
    ...
    fun `convert kg to lb`() = runBlocking {
        testedValues.forEach { testedValue ->
            testedValue.convert(from = MassUnit.Kilograms, to = MassUnit.Pounds)
        }
    }
    ...
    fun `convert lb to lb`() = runBlocking {
        testedValues.forEach { testedValue ->
            testedValue.convert(from = MassUnit.Pounds, to = MassUnit.Pounds)
        }
    }
    ...
    fun `convert lb to kg`() = runBlocking {
        testedValues.forEach { testedValue ->
            testedValue.convert(from = MassUnit.Pounds, to = MassUnit.Kilograms)
        }
    }
    ...
    fun `convert km to km`() = runBlocking {
        testedValues.forEach { testedValue ->
            testedValue.convert(from = LongDistanceUnit.Kilometer, to = LongDistanceUnit.Kilometer)
        }
    }
    ...
    fun `convert km to mi`() = runBlocking {
        testedValues.forEach { testedValue ->
            testedValue.convert(from = LongDistanceUnit.Kilometer, to = LongDistanceUnit.Mile)
        }
    }
    ...
    fun `convert mi to mi`() = runBlocking {
        testedValues.forEach { testedValue ->
            testedValue.convert(from = LongDistanceUnit.Mile, to = LongDistanceUnit.Mile)
        }
    }
    ...
    fun `convert mi to km`() = runBlocking {
        testedValues.forEach { testedValue ->
            testedValue.convert(from = LongDistanceUnit.Mile, to = LongDistanceUnit.Kilometer)
        }
    }
