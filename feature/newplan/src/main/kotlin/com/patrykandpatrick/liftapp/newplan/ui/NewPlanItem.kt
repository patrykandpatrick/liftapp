...
fun NewPlanItem(
    ...
    onClick: (NewPlanState.Item.PlanElement) -> Unit,
    modifier: Modifier = Modifier,
) {
    OutlinedCard(
        onClick = { if (item is NewPlanState.Item.PlanElement) onClick(item) },
        modifier = modifier,
    ) {
        AnimatedContent(item) { planItem ->
            when (planItem) {
                NewPlanState.Item.PlaceholderItem ->
                    PlaceholderItem(onAddRestDayClick, onAddRoutineClick)
                is NewPlanState.Item.RestItem -> RestItem()
                is NewPlanState.Item.RoutineItem -> RestItemRoutine(planItem.routine)
            }
        }
    }
}
...
@Composable
private fun NewPlanItemPreview(item: NewPlanState.Item) {
    LiftAppTheme {
        Surface {
            NewPlanItem(
                item = item,
...
private fun NewPlanPlaceholderItemPreview() {
    NewPlanItemPreview(NewPlanState.Item.PlaceholderItem)
}

@Composable
@LightAndDarkThemePreview
private fun NewPlanRestItemPreview() {
    NewPlanItemPreview(NewPlanState.Item.RestItem())
}
