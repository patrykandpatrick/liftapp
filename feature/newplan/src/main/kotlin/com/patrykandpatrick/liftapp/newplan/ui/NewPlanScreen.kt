...
@Composable
fun NewPlanScreen(modifier: Modifier = Modifier) {
    val viewModel: NewPlanViewModel = hiltViewModel()

    val state = viewModel.state.collectAsState().value

    NewPlanScreen(state, viewModel::onAction, modifier)
}

@Composable
private fun NewPlanScreen(
    loadableState: Loadable<NewPlanState>,
...
private fun LazyListScope.items(state: NewPlanState, onAction: (Action) -> Unit) {
    item(key = "input") {
        EditableDetails(
            state,
...
@Composable
private fun EditableDetails(state: NewPlanState, modifier: Modifier = Modifier) {
    Column(
        modifier = modifier,
...
@Composable
private fun NewPlanItem(
    index: Int,
    item: NewPlanState.Item,
    onAction: (Action) -> Unit,
    modifier: Modifier = Modifier,
) {
    Row(
        ...
        DayIndicator(dayIndex = index, enabled = item !is NewPlanState.Item.PlaceholderItem)
        ...
        if (item is NewPlanState.Item.PlaceholderItem) {
            ...
            IconButton(
                ...
                modifier =
                    Modifier.align(
                        if (item is NewPlanState.Item.RestItem) Alignment.CenterVertically
                        else Alignment.Top
                    ),
            ) {
                Icon(
