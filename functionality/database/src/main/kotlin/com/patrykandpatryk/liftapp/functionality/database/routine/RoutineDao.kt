...
interface RoutineDao {
    ...
    @Query(
        value =
            "SELECT exercise.*, goal.* from exercise_with_routine as ewr " +
    ...
    @Insert suspend fun insert(routine: RoutineEntity): Long

    @Insert suspend fun insert(exerciseWithRoutine: ExerciseWithRoutineEntity)

    @Upsert suspend fun upsert(exerciseWithRoutine: List<ExerciseWithRoutineEntity>)

    @Query(
        "DELETE FROM exercise_with_routine WHERE routine_id = :routineId AND exercise_id = :exerciseId"
    )
    suspend fun deleteExerciseWithRoutine(routineId: Long, exerciseId: Long)

    @Query(
        "DELETE FROM exercise_with_routine WHERE  routine_id = :routineId AND exercise_id NOT IN (:notIn)"
    )
